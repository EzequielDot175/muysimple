...
var galerias = [];
var indexOfGaleria = [];
$.each($('.SegundaGaleriaswiper-container'), function(index, val) {
    console.info('Reporting :', val.classList[1]);
    galerias.push(new Swiper('.'+val.classList[1],{
      pagination: '.pagination',
      paginationClickable: true,
      slidesPerView: 'auto'
    }) );
    indexOfGaleria.push(val.classList[1]);
});

$('.arrow-left4').click(function(event) {
    var parent = $(this).parents('.SegundaGaleriaswiper-container');
        parentClassName = parent[0].classList[1];
        index = indexOfGaleria.indexOf(parentClassName);

        galerias[index].swipePrev();
});

$('.arrow-right4').click(function(event) {
    var parent = $(this).parents('.SegundaGaleriaswiper-container');
        parentClassName = parent[0].classList[1];
        index = indexOfGaleria.indexOf(parentClassName);

        galerias[index].swipeNext();
});
...
$(document).ready(function() {
  var container = $('.ListaEmpresas');
  var list = container.children('ul');
  var items = list.children('li');

  /**
   * Idea: Calcular el width de cada item para indicarle al contendor que ancho en pixeles deberia tener, de esta manera se podria centrar de forma dinamica
   */
